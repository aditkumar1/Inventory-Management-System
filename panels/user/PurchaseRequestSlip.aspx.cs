...
public partial class Store_ProductEntrySearchSlip : System.Web.UI.Page
{
     ...
     protected void BindField()
    {
        ...
        Label2.Text = dt.ToString(format);
         if(!Userid.Equals(""))
        {
            string strQuery = "select Requester,Depname,Status,Convert(varchar(10),CONVERT(date,Gendate,106),103) as Gendate,Convert(varchar(10),CONVERT(date,Expdate,106),103) as Expdate,ForwardDesignation as Approver from PurchaseRequisition,PurchaseRequisitionTrack where PurchaseRequisitionTrack.Sno=(Select Max(Sno) from PurchaseRequisitionTrack where PurchaseRequisitionTrack.Prno=PurchaseRequisition.Prno) and PurchaseRequisition.Prno=" + Session["SelectedSlipno"].ToString(); 
            ...
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    Label8.Text = dr["Expdate"].ToString();
                    Label9.Text = dr["Gendate"].ToString();
    ...
    {
        string strQuery = "select_purchase_requisition"; 
        DataTable dt = new DataTable();
        String strConnString = System.Configuration.ConfigurationManager.ConnectionStrings["MainConnection"].ConnectionString;
        SqlConnection con = new SqlConnection(strConnString);
        SqlDataAdapter sda = new SqlDataAdapter();
        SqlCommand cmd = new SqlCommand(strQuery);
        cmd.Parameters.AddWithValue("@Prno", Session["SelectedSlipno"].ToString());
    ...
    */
    ...
    {
        Response.Redirect("PendingPurchaseRequest.aspx");
    }
