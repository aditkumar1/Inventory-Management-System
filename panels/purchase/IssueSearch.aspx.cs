...
public partial class ProductEntryModify : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if(RadioButton2.Checked)
        {
            ...
            string strQuery = "select DISTINCT(Isno),Depname,Isby,CONVERT(varchar(10),IssueDetails.Date) as Date from IssueDetails where Isno LIKE '%" + TextBox1.Text + "%'";
            BindGrid(strQuery);
        }
        if(RadioButton3.Checked)
        {
            ...
            TextBox3.Enabled = false;
            TextBox4.Enabled = false;
            string strQuery = "select DISTINCT(Isno),Isby,Depname,CONVERT(varchar(10),IssueDetails.Date) as Date from IssueDetails where Depname LIKE '%" + TextBox2.Text + "%'";
            BindGrid(strQuery);
        }
        if(RadioButton4.Checked)
        {
            ...
            string strQuery = "select DISTINCT(Isno),Isby,Depname,CONVERT(varchar(10),IssueDetails.Date) as Date from IssueDetails where Date BETWEEN '" + TextBox3.Text + "' AND  '"+TextBox4.Text+"'";
            BindGrid(strQuery);
        }
    }
