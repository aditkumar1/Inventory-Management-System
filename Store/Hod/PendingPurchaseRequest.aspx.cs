...
public partial class Store_Hod_PendingIssueRequest : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        GetSelectedRecord();
         if (RadioButton3.Checked)
         {
            ...
            string strQuery = "select DISTINCT(PurchaseRequisition.Prno) as Prno,Status,'Forwarded to '+PurchaseRequisitionTrack.ForwardDesignation as ForwardDesignation,Apprvremarks as Remarks,Convert(varchar(10),CONVERT(date,Gendate,106),103) as Date,Convert(varchar(10),CONVERT(date,Expdate,106),103) as Expdate,Gendate as Sortdate from PurchaseRequisition,PurchaseRequisitionTrack where PurchaseRequisitionTrack.Sno=(Select Max(Sno) from PurchaseRequisitionTrack where PurchaseRequisitionTrack.Prno=PurchaseRequisition.Prno)  and PurchaseRequisition.Prno LIKE '%" + TextBox2.Text + "%' and Requester='" + Requester + "' and Depname='" + Depname + "' and Designation='" + Designation + "' Order By Sortdate DESC";
            BindGrid(strQuery);
        }
        if (RadioButton4.Checked)
        {
            ...
            string strQuery = "select DISTINCT(PurchaseRequisition.Prno) as Prno,Status,'Forwarded to '+PurchaseRequisitionTrack.ForwardDesignation as ForwardDesignation,Apprvremarks as Remarks,Convert(varchar(10),CONVERT(date,Gendate,106),103)  as Date,Gendate as Sortdate,Convert(varchar(10),CONVERT(date,Expdate,106),103) as Expdate from PurchaseRequisition,PurchaseRequisitionTrack where PurchaseRequisitionTrack.Sno=(Select Max(Sno) from PurchaseRequisitionTrack where PurchaseRequisitionTrack.Prno=PurchaseRequisition.Prno)  and Gendate BETWEEN '" + TextBox3.Text + "' AND  '" + TextBox4.Text + "' and Requester='" + Requester + "' and Depname='" + Depname + "' and Designation='" + Designation + "' Order By Sortdate DESC";
            BindGrid(strQuery);
        }
    }
    ...
    protected void Button1_Click1(object sender, EventArgs e)
    {
        ...
        if (ViewState["SelectedSno"] != null)
        {
            ...
            Response.Redirect("PurchaseRequestSlip.aspx");
        }
    }
}